...
<body id="mainBody" data-ctrl=true style="background:#f8f8f8;">
    <div id="mainDiv" style="width: 100%;height:100%;">
	   <!--  <header data-am-widget="header" class="am-header am-header-default">
		...
		<div class="checkCode">
        	<div style="width:80%;"><input type="text" placeholder="请设置登录密码" id="password" value="" ></div>
        </div>
		<div class="checkCode">
        	<div style="width:80%;"><input style="border:none;" type="text" placeholder="请再次设置登录密码" id="password_again" value="" ></div>
        </div>
...
<script type="text/javascript">
		...
		//	val = '15920200230';
			if(val){
				...
				$.ajax({
					...
					data:{op:"send",phone:val,customer_id:customer_id},
					dataType:'json',
					type:'post',
					beforeSend:function(){
					  send_obj.attr('disabled',"disabled");
	...
	function comfirm(){
		...
		/*//输入密码
		if(check_illegalchar(password_again) || check_illegalchar(password)){//检查密码
			showAlertMsg ("提示：","您输入的是非法字符!请重新输入!","知道了");
			return false;
		}
		if(password == '' || password_again == ''){
			showAlertMsg ("提示：","密码不能为空","知道了");
			return false;
		}*/

		//验证登陆密码
		var passwords_rtn = check_login_password(password,'登陆密码');
		...
		$.ajax({
			url:'checkuser_login.php',
			data:{op:"edit",yzm: check_code,customer_id:customer_id,phone:phone,password:password},
			dataType:'json',
			type:'post',
